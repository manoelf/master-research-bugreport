...
class Finetunning:
    ...
    def model_finetuning(self, model: Model, model_name: ModelName, x_train, y_train, x_test, y_test, classes, model_turned_metrics_path, model_pred_path, folds=10):
        ...
        with open(model_turned_metrics_path, 'wb') as model_file:
            pickle.dump(model_tuned_metrics, model_file)

        with open(model_pred_path, 'wb') as model_file:
            pickle.dump(model_pred, model_file)
